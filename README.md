...
```text
    ...
    !!! При описании теста, обращающегося к view-функциям с рендерингом форм, необходимо отключать передачу CSRF-токена !!!
```
## Ключевые элементы
```text
    ...
    37) @pytest.fixture         - превращение функции в фиксутуру для настройки тестового клиента Flask;
    38) def client() -> FlaskClient:        - функция для настройки тестового клиента Flask;
    39) with app.test_client() as test_client:      - получение тестового клиента;
    40) with app.app_context():     - получение контекста тестового клиента;
    41) app.config.update(WTF_CSRF_ENABLED=False)       - отключение проверки CSRF-токена для тестирования с помощью тестового клиента;
```
## Шаги 
```text
    ...
    30) Из 25го урока добавили форму для вывода данных по товару, а так же view-функцию добавления товара для тестирования функции с рендерингом формы;
    31) Добавили новый тест для добавленной view-функции, при запуске получили ошибку: "AssertionError: <h1>eroror: 404 Not Found... assert 200 == 302...";
    32) Изменили адрес обращения при запуске теста (добавили "/products/") из префикса Blueprint'а, запустили тест, получили ошибку:
        "RuntimeError: A secret key is required to use CSRF." - т.к. форма должна передаваться на валидацию с токеном;
    33) В настройках тестового клиента добавили настройку, отключающую проверку CSRF-токена;
    34) Запустили тест, выполнился без ошибок;
    35) Проверили линтером добавленный и изменённые файлы, исправили обнаруженные отклонения от PEP8;
    36) Закоммитили изменения и запушили их в репозиторий, Actions отработали успешно;
    37) 
    38) 
    39) 
    40) 
